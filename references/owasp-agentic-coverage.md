...
| ASI02 | Excessive Agency / Tool Misuse | COVERED — risk-scored decision engine routes all tool calls; workflow-router constrains targets; `ECC_ENFORCE=1` exits code 2 to block; trajectory nudge limits runaway sessions | `runtime/decision-engine.js`, `runtime/workflow-router.js`, `claude/hooks/dangerous-command-gate.js` |
...
| ASI04 | Sensitive Information Disclosure | PARTIAL — `secret-warning.js` detects 23 secret patterns before Bash tool calls and blocks in `ECC_ENFORCE=1`; hook log records metadata only, never content; path-sensitivity classifier flags high-sensitivity paths. `redact-payload.sh` is an offline audit tool — not wired into hook execution | `claude/hooks/secret-warning.js`, `claude/hooks/hook-utils.js`, `scripts/redact-payload.sh` |
| ASI05 | Improper Output Handling | PARTIAL — `audit-examples.sh` scans prose and GOOD blocks for dangerous patterns; `audit-local.sh` scans scripts and hooks; no post-tool output sanitisation hook | `scripts/audit-examples.sh`, `scripts/audit-local.sh` |
...
| ASI08 | Uncontrolled Agentic Loops / Cascading Failures | COVERED — token-bucket rate limiter caps hook invocations per second; session-trajectory routing escalates after repeated risky decisions; `ECC_KILL_SWITCH=1` halts all decisions | `claude/hooks/hook-utils.js` (`rateLimitCheck`), `runtime/session-context.js` (`getSessionTrajectory`), `runtime/decision-engine.js` |
...
| ASI10 | Rogue Agents / Uncontrolled Autonomy | COVERED — `ECC_KILL_SWITCH=1` blocks all decisions immediately; policy-store prevents auto-promotion without operator approval; auto-allow-once is single-use and eligible-gated | `runtime/decision-engine.js` (`ECC_KILL_SWITCH`), `runtime/policy-store.js` |
...
- Post-tool output sanitisation (ASI05): Claude Code does not expose a PostToolUse hook that
  receives tool output text; not implementable at this layer.
